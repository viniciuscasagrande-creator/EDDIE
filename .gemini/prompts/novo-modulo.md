...
> Cole no chat do Gemini Code Assist. Substitua `<MODULO>` e preencha o contexto.
...
exatamente o padrão já existente em `modules/eventos/`.
...
1. `packages/contracts/src/events/<modulo>.ts` — os eventos com Zod, usando
   `defineEvent`. Registre-os no `EventCatalog` em `packages/contracts/src/index.ts`.
2. Os models Prisma no schema `<modulo>` dentro de `apps/api/prisma/schema.prisma`.
   Lembre: IDs de outros módulos são escalares, SEM `@relation`.
3. `<modulo>.service.ts` — lógica de negócio. Toda mudança de estado dentro de
   `$transaction` + `outbox.emit`.
...
Não invente nomes de eventos: use os do `EventCatalog`. Se precisar de um novo,
me pergunte antes.
