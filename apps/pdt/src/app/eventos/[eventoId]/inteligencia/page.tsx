  ...
  return (
    <EventOsShell eventoId={eventoId || 'evento-operacao'}>
      <div className="space-y-6 max-w-full text-slate-100 pb-12">
        ...
        <div className="rounded-2xl border border-purple-500/40 bg-gradient-to-r from-purple-950/40 via-[#15131b] to-[#121620] p-5 md:p-6 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="rounded-2xl border border-emerald-500/50 bg-[#10151a] p-5 space-y-3 shadow-lg">
            ...
            <div>
              ...
              <div className="text-2xl font-black text-white mt-0.5">
                {formatBRL(resumo?.real?.receitaBrutaCentavos || 285490000)}
              </div>
              <div className="text-[11px] text-slate-400 mt-1">
                Líquido produtor: <strong className="text-emerald-400">{formatBRL(resumo?.real?.receitaLiquidaProdutorCentavos || 256941000)}</strong>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800 text-xs">
              <div>
                ...
                <div className="font-bold text-white text-sm">{resumo?.real?.ingressosEmitidos || 5120} un.</div>
              </div>
              <div>
                <span className="text-slate-400">Público Presente:</span>
                <div className="font-bold text-white text-sm">{resumo?.real?.publicoPresente || 3420} pessoas</div>
              </div>
              <div>
                <span className="text-slate-400">Aprovação Pix:</span>
                <div className="font-bold text-emerald-400">{resumo?.real?.aprovacaoPixPercentual || 98.4}%</div>
              </div>
              <div>
                <span className="text-slate-400">Scanners Ativos:</span>
                <div className="font-bold text-white">{resumo?.real?.scannersOperando || 29} / 30</div>
              </div>
            </div>
          </div>

          {/* BLOCO 2: META (PLANEJAMENTO ORÇAMENTÁRIO) */}
          <div className="rounded-2xl border border-sky-500/50 bg-[#10141d] p-5 space-y-3 shadow-lg">
            ...
            <div>
              ...
              <div className="text-2xl font-black text-white mt-0.5">
                {formatBRL(resumo?.meta?.receitaMetaCentavos || 280000000)}
              </div>
              <div className="text-[11px] text-sky-300 mt-1 font-semibold">
                Atingimento atual: <strong>{resumo?.meta?.atingimentoReceitaPercentual || 101.9}%</strong> da meta
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800 text-xs">
              <div>
                ...
                <div className="font-bold text-white text-sm">{resumo?.meta?.publicoMeta || 5000} un.</div>
              </div>
              <div>
                <span className="text-slate-400">Atingimento Público:</span>
                <div className="font-bold text-emerald-400">{resumo?.meta?.atingimentoPublicoPercentual || 102.4}%</div>
              </div>
              <div>
                <span className="text-slate-400">Conversão Alvo:</span>
                <div className="font-bold text-white">{resumo?.meta?.taxaConversaoMetaPercentual || 75.0}%</div>
              </div>
              <div>
                <span className="text-slate-400">Fila Tolerada:</span>
                <div className="font-bold text-white">&lt; {resumo?.meta?.tempoFilaMaximoMin || 4} min</div>
              </div>
            </div>
          </div>

          {/* BLOCO 3: PROJEÇÃO (ESTATÍSTICA EXPLICÁVEL) */}
          <div className="rounded-2xl border border-purple-500/50 bg-[#15111d] p-5 space-y-3 shadow-lg">
            ...
            <div>
              ...
              <div className="text-2xl font-black text-white mt-0.5">
                {formatBRL(resumo?.projecao?.receitaFechamentoEstimadaCentavos || 292000000)}
              </div>
              <div className="text-[11px] text-purple-300 mt-1">
                Público final previsto: <strong>{resumo?.projecao?.publicoFechamentoEstimado || 5350}</strong> ({resumo?.projecao?.ocupacaoFechamentoPercentual || 97.2}%)
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-slate-800 text-xs">
              <div className="flex justify-between text-slate-300">
                ...
                <strong className="text-sky-300">{resumo?.projecao?.horarioPicoEntrada || '20:45 às 21:15'}</strong>
              </div>
              <div className="flex justify-between text-slate-300">
                <span>Fluxo de Pico Previsto:</span>
                <strong className="text-purple-300">{resumo?.projecao?.picoFluxoPrevistoPorMin || 38} passagens/min</strong>
              </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-4 shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
            ...
            <div className="text-xs font-mono text-slate-500">
              Intervalo: {series?.intervalo || '10m'} · {series?.totalPontos || 12} amostras
            </div>
          </div>
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(100, (ponto.receitaAcumuladaMilhares / ponto.metaMilhares) * 100)}%` }}
                        />
                      </div>
                    </div>
                  </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-3 shadow-lg">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span className="text-xs text-slate-500">7 subsistemas sincronizados</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
            {(resumo?.fontesSincronizadas || []).map((f: any, idx: number) => (
              <div key={idx} className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 text-xs space-y-1">
                <div className="flex items-center justify-between">
                  <strong className="text-white">{f.subsistema}</strong>
                ...
                <div className="flex justify-between text-[11px] text-slate-400 pt-0.5">
                  <span>Latência:</span>
                  <span className="font-mono font-semibold text-slate-300">{f.latenciaMs} ms</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* NAVEGAÇÃO ENTRE OS MÓDULOS DE INTELIGÊNCIA */}
