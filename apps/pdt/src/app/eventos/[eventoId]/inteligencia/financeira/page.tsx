        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-1 shadow-lg">
            ...
            <div className="text-2xl font-black text-white mt-1">
              {formatBRL(dados?.gmvTotalCentavos || 285490000)}
            </div>
          ...
          <div className="rounded-2xl border border-emerald-500/40 bg-gradient-to-br from-emerald-950/30 to-[#121620] p-5 space-y-1 shadow-lg">
            ...
            <div className="text-2xl font-black text-white mt-1">
              {formatBRL(dados?.direitosProdutor?.valorLiquidoTotalCentavos || 251231200)}
            </div>
          ...
          <div className="rounded-2xl border border-sky-500/40 bg-[#121620] p-5 space-y-1 shadow-lg">
            ...
            <div className="text-2xl font-black text-sky-300 mt-1">
              {formatBRL(dados?.direitosProdutor?.disponivelParaRepasseCentavos || 81231200)}
            </div>
          ...
          <div className="rounded-2xl border border-amber-500/40 bg-[#121620] p-5 space-y-1 shadow-lg">
            ...
            <div className="text-2xl font-black text-amber-300 mt-1">
              {formatBRL(dados?.direitosProdutor?.retencaoSegurancaCdcCentavos || 20000000)}
            </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-3 shadow-lg">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="text-xs font-mono font-bold text-sky-300 bg-sky-950/50 px-2 py-0.5 rounded border border-sky-800/40">
                {dados?.receitaPropriaDiskIngressos?.percentualContratual || 10}% Contratual
              </span>
            </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between text-slate-300">
                ...
                <strong className="text-white font-mono text-sm">{formatBRL(dados?.receitaPropriaDiskIngressos?.valorCentavos || 28549000)}</strong>
              </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-3 shadow-lg">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="text-xs font-mono font-bold text-amber-300 bg-amber-950/50 px-2 py-0.5 rounded border border-amber-800/40">
                {formatBRL(dados?.custosGatewayAdquirencia?.custoTotalCentavos || 5709800)}
              </span>
            </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between text-slate-300">
                ...
                <strong className="text-slate-200">{formatBRL(dados?.custosGatewayAdquirencia?.processamentoPixCentavos || 1420000)}</strong>
              </div>
              <div className="flex justify-between text-slate-300">
                <span>Processamento Cartão de Crédito / Antifraude:</span>
                <strong className="text-slate-200">{formatBRL(dados?.custosGatewayAdquirencia?.processamentoCartaoCentavos || 4289800)}</strong>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-4 shadow-lg">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span>Cronograma & Extrato de Repasses ao Produtor</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Controle de adiantamentos, liquidações bancárias programadas e liberação de reservas técnicas.
              </p>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/30">
                Conciliação Bancária: {dados?.conciliacaoBancaria?.percentualConciliado || 99.8}%
              </span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {(dados?.cronogramaRepasses || []).map((rep: any) => {
              ...
              const isBloqueado = rep.status === 'BLOQUEADO_TEMPORARIO';

              return (
                <div
                  key={rep.id}
                  className="rounded-xl border border-slate-800 bg-slate-900/70 p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs"
                >
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <strong className="text-white text-sm">{rep.descricao}</strong>
                    </div>

                    <div className="text-slate-400">
                      Destino: <span className="text-slate-200">{rep.bancoDestino || 'Conta Principal Produtor'}</span>
                      {rep.dataLiquidacao && (
                        <span> · Liquidado em {new Date(rep.dataLiquidacao).toLocaleDateString('pt-BR')}</span>
                      )}
                      {rep.dataPrevista && (
                        <span> · Previsão: {new Date(rep.dataPrevista).toLocaleDateString('pt-BR')}</span>
                      )}
                    </div>

                    {rep.condicaoLiberacao && (
                      <div className="text-[11px] text-amber-300">
                        Condição: {rep.condicaoLiberacao}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right">
                      <div className="text-base font-black text-white font-mono">{formatBRL(rep.valorCentavos)}</div>
                    </div>
