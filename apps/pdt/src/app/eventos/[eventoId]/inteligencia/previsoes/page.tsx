        ...
        {aba === 'VENDAS' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-2">
                ...
                <div className="text-2xl font-black text-white">
                  {formatBRL(dadosVendas?.projecaoFechamento?.cenarioConservadorCentavos || 288000000)}
                </div>
                <p className="text-xs text-slate-400">
              ...
              <div className="rounded-2xl border border-purple-500/50 bg-gradient-to-br from-purple-950/30 to-[#121620] p-5 space-y-2 shadow-lg">
                ...
                <div className="text-2xl font-black text-white">
                  {formatBRL(dadosVendas?.projecaoFechamento?.cenarioBaseCentavos || 292000000)}
                </div>
                <p className="text-xs text-purple-200/80">
                  Probabilidade de atingir a meta: <strong>{dadosVendas?.projecaoFechamento?.probabilidadeAtingirMetaPercentual || 96.8}%</strong>.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-2">
                ...
                <div className="text-2xl font-black text-emerald-300">
                  {formatBRL(dadosVendas?.projecaoFechamento?.cenarioOtimistaCentavos || 296500000)}
                </div>
                <p className="text-xs text-slate-400">
                  Com aceleração de bilheteria física até as 22h.
                </p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-4 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {(dadosVendas?.lotesPrevisao || []).map((lote: any) => (
                  <div
                    key={lote.loteId}
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-bold text-white text-sm">{lote.nome}</h4>
                        <div className="text-xs text-slate-400">{lote.ingressosRestantes} ingressos restantes</div>
                      </div>
            ...
            {/* COMPARATIVO PREVISTO vs REALIZADO (ÚLTIMAS HORAS) */}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-2">
                ...
                <div className="text-2xl font-black text-white">
                  {dadosPortaria?.portariaReal?.publicoEsperado || 5350}
                </div>
                <div className="text-xs text-slate-400">
                  {dadosPortaria?.portariaReal?.totalEntradas || 3420} já passaram pela catraca.
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-sky-500/50 bg-[#10141e] p-5 space-y-2 shadow-lg">
                ...
                <div className="text-2xl font-black text-white">
                  {dadosPortaria?.projecaoFluxo?.horarioPicoEstimado || '20:45 às 21:15'}
                </div>
                <div className="text-xs text-sky-300">
                  Fluxo máximo previsto: <strong>{dadosPortaria?.projecaoFluxo?.taxaPicoEsperadaMin || 38} pessoas/min</strong>.
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-[#121620] p-5 space-y-2">
                ...
                <div className="text-2xl font-black text-amber-300">
                  {dadosPortaria?.projecaoFluxo?.tempoFilaPicoMinutos || 3.4} min
                </div>
                <div className="text-xs text-slate-400">
                  Normalização esperada às {dadosPortaria?.projecaoFluxo?.horaEstimadaNormalizacao || '21:40'}.
                </div>
              </div>
            </div>
