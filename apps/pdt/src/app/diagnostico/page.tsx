...
type Operational = { ok?:boolean; stage?:string; produtorId?:string; tenantId?:string; eventoId?:string; totalEventos?:number; code?:string; message?:string; tenantAutocorrected?:boolean; durationMs?:number; checks?:Array<{name:string;ok:boolean;http?:number;status?:string;autocorrected?:boolean}> };
...
export default function DiagnosticoPage() {
  ...
  const carregar = async () => {
    setLoading(true);
    try { const [r,c] = await Promise.all([fetch('/api/status', { cache: 'no-store' }), fetch('/api/bootstrap', { cache: 'no-store' })]); setStatus(await r.json()); setOperational(await c.json()); }
  ...
  return <div className="p-6 lg:p-8 max-w-6xl mx-auto text-slate-100 space-y-5">
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5 space-y-3"><h2 className="font-bold flex items-center gap-2"><Server size={18}/>Contexto operacional</h2><div className="grid md:grid-cols-4 gap-3 text-sm"><Info label="Produtor" value={ctx.produtorId || operational.produtorId || 'Não configurado'} /><Info label="Tenant" value={ctx.tenantId || operational.tenantId || 'Não configurado'} /><Info label="Eventos carregados" value={String(operational.totalEventos ?? ctx.eventos.length)} /><Info label="Evento selecionado" value={ctx.evento?.nome || 'Nenhum'} /></div>{operational.tenantAutocorrected && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-200 p-3 text-sm">Tenant autocorrigido a partir do cadastro real do produtor.</div>}{operational.checks?.length ? <div className="grid md:grid-cols-3 gap-2">{operational.checks.map((c)=><div key={c.name} className={`rounded-lg border p-3 text-xs ${c.ok?'border-emerald-500/20 bg-emerald-500/5 text-emerald-300':'border-rose-500/20 bg-rose-500/5 text-rose-300'}`}><b>{c.ok?'OK':'FALHA'}</b> · {c.name}{c.http?` · HTTP ${c.http}`:''}{c.autocorrected?' · autocorrigido':''}</div>)}</div>:null}{operational.message && <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 text-cyan-200 p-3 text-sm">Contexto: {operational.message}{operational.code ? ` · ${operational.code}` : ''}</div>}{ctx.error && <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 text-rose-300 p-3 text-sm">{ctx.error}</div>}{status.message && <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-200 p-3 text-sm">{status.message}</div>}</section>
    <section className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5"><h2 className="font-bold mb-3">Configuração obrigatória na Vercel</h2><div className="font-mono text-xs text-slate-300 space-y-2"><div>API_INTERNAL_URL=https://SEU-BACKEND</div><div>NEXT_PUBLIC_API_URL=/api</div><div>PRODUTOR_ID=UUID_REAL_DO_PRODUTOR</div><div>TENANT_ID=opcional — o 11.2 resolve pelo cadastro real do produtor</div><div>NEXT_PUBLIC_PRODUTOR_ID=UUID_REAL_DO_PRODUTOR (fallback)</div></div><p className="text-xs text-slate-500 mt-3">API_INTERNAL_URL pode terminar com /api ou apenas com o domínio; o proxy normaliza automaticamente.</p></section>
